...
### עדכון פלייליסט תוך כדי נגינה

הסוכן שולף פלייליסט מהשרת פעם אחת, כשהנגינה מתחילה, ולא שולף מחדש פלייליסט שכבר טעון
בזיכרון — גם לא כששולחים לו `play` עם אותו מזהה. לכן שינוי בתוכן הפלייליסט בשרת (רענון
מיוטיוב שמסיר רצועות, מחיקת שיר ידנית, או המתזמן השבועי) לא מגיע מעצמו למכשיר, וכל רצועה
שהוסרה עולה כשש עד שבע שניות שקט: הסוכן מנסה לפענח אותה ב-yt-dlp, נכשל, ורק אז מדלג.

שתי הגנות עובדות כאן יחד:

- **השרת דוחף `playback_control` עם `action: "reload_playlist"`** לכל סניף ששדה
  `current_playlist_id` שלו מצביע על הפלייליסט שהשתנה. הקאש של הפלייליסט מנוקה **לפני**
  שליחת הפקודה, אחרת המכשיר היה שולף בחזרה בדיוק את הרשימה הישנה.
- **הסוכן מרענן את הרשימה בעצמו** אחרי שלוש רצועות רצופות שנכשלו בפענוח (עם רצפה של חמש
  דקות בין רענונים), כך שגם פקודה שאבדה בזמן שהמכשיר היה מנותק לא מצטברת לשקטים.

הטעינה מחדש לא עוצרת את הנגינה: הרצועה המתנגנת ממשיכה, והסמן עובר למקום שלה ברשימה החדשה.
אם הרצועה המתנגנת עצמה הוסרה, היא מתנגנת עד סופה והרצועה הבאה נבחרת מהרשימה המעודכנת.
בקשה שנכשלה או רשימה שחזרה ריקה לא מוחקות את מה שטעון — עדיף פלייליסט ישן מהפסקת נגינה.
